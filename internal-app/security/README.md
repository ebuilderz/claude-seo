...
The production audit profile exposes bubblewrap's fresh, sandbox-private `/proc` read-only so Playwright Chromium can inspect its own process metadata. The parent Codex process is outside that PID namespace, the command environment excludes the API key, and the domain allowlist plus all outer profiles remain enforced.
