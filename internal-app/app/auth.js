...
function assertAllowedIdentity(email, allowedEmails, allowedDomains) {
...
export function createAuth(options = {}) {
  ...
  const mode = String(env.AUTH_MODE || (production ? "cloudflare-access" : "none")).toLowerCase();
  ...
  if (mode === "none" && production && env.ALLOW_UNSAFE_NO_AUTH !== "true") {
    throw new Error("AUTH_MODE=none is blocked in production. Configure Cloudflare Access or basic auth.");
  }
  ...
  if (mode === "basic") {
    ...
    return (req, res, next) => {
      ...
      if (!header.startsWith("Basic ")) {
        res.set("WWW-Authenticate", 'Basic realm="eBuilderz SEO Audit", charset="UTF-8"');
        return reject(res);
      }
  ...
  if (mode === "cloudflare-access") {
    ...
    return async (req, res, next) => {
      ...
      try {
        ...
        if (!assertAllowedIdentity(email, allowedEmails, allowedDomains)) {
          return reject(res, 403, "This account is not allowed to use the SEO audit workspace.");
        }
